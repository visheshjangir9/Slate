...
const STAGES: { key: RunState['stage']; label: string; detail: string }[] = [
  { key: 'image', label: 'Composing frame', detail: 'Generating the source still' },
  { key: 'render', label: 'Moving camera', detail: 'Applying the camera move' },
  { key: 'encode', label: 'Encoding', detail: 'Writing H.264 frames' },
  { key: 'upload', label: 'Saving', detail: 'Storing the finished clip' },
]

const fmtBytes = (n: number) => (n < 1e6 ? `${(n / 1e3).toFixed(0)} KB` : `${(n / 1e6).toFixed(2)} MB`)

function Frame({ aspect, children }: { aspect: number; children: React.ReactNode }) {
  return (
    <div className="flex h-full w-full items-center justify-center p-4 sm:p-8">
      <div
        className="grain relative w-full overflow-hidden rounded-card border border-line bg-surface"
        style={{ aspectRatio: aspect, maxHeight: '100%', maxWidth: `min(100%, calc((100cqh - 4rem) * ${aspect}))` }}
      >
        {children}
      </div>
    </div>
...
export function Stage({
  generation, run, onRetry, onReuse,
}: {
  ...
  if (generation.status === 'failed') {
    return <FailedStage generation={generation} aspect={aspect} onRetry={onRetry} />
  }
  return <ResultStage generation={generation} aspect={aspect} onReuse={onReuse} />
}
...
function EmptyStage() {
  return (
    <div className="flex h-full flex-col items-center justify-center px-6 text-center">
      <div
        className="mb-6 h-px w-40"
        style={{ background: 'linear-gradient(90deg, transparent, var(--color-line-strong), transparent)' }}
      />
      <h1 className="text-[22px] font-semibold tracking-[-0.02em] text-ink sm:text-2xl">
        Write the shot. Pick the move.
      </h1>
      <p className="mt-2.5 max-w-sm text-sm leading-relaxed text-ink-3">
        Slate turns a prompt into a real cinematic clip — an AI-generated frame driven by a
        genuine camera move and encoded as H.264 video you can download.
      </p>
      <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
        {['Dolly In', 'Crash Zoom', 'Orbit', 'Crane Up', 'Handheld'].map((m) => (
          <span key={m} className="rounded-full border border-line px-2.5 py-1 text-[11px] text-ink-3">
            {m}
          </span>
        ))}
      </div>
    ...
    <div className="flex h-full flex-col">
      <Frame aspect={aspect}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
          ...
          <span className="tabular text-xs text-ink-2">{Math.round(pct)}%</span>
        </div>
      </Frame>
...
function FailedStage({
  generation, aspect, onRetry,
}: { generation: Generation; aspect: number; onRetry: (id: string) => void }) {
      ...
      <Frame aspect={aspect}>
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-6 text-center">
          ...
          <div>
            ...
            <p className="mt-1.5 max-w-sm text-xs leading-relaxed text-ink-3">{copy}</p>
          </div>
          <button
            type="button"
...
function ResultStage({
  generation, aspect, onReuse,
}: { generation: Generation; aspect: number; onReuse: (g: Generation) => void }) {
  const dims = dimensionsFor(generation.aspectRatio, generation.resolution)
  return (
    <div className="flex h-full flex-col">
      <Frame aspect={aspect}>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <div className="tabular mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-ink-4">
              ...
              {generation.fileBytes && <span>{fmtBytes(generation.fileBytes)}</span>}
