...
import { readMarkers, withMarkers } from './origin'
import type {
...
export class SupabaseGenerationStore implements GenerationStore {
  ...
  async create(input: NewGeneration): Promise<Generation> {
    ...
    const payload = {
      ...
      adjustments: withMarkers(input.adjustments ?? [], { workflow: input.workflow, providerJob: input.providerJob }),
  ...
  async patch(id: string, patch: Partial<Generation>): Promise<Generation | null> {
    ...
    delete row.id
    // Markers share the adjustments column: merge, never overwrite, so setting
    // a job handle cannot erase the workflow marker (or the reverse).
    if ('providerJob' in patch || 'adjustments' in patch || 'workflow' in patch) {
      const { data: cur, error: readErr } = await supabase().from(TABLE).select('adjustments').eq('id', id).maybeSingle()
      if (readErr) boom('select', readErr)
      if (!cur) return null
      const now = readMarkers((cur as { adjustments: Adjustment[] | null }).adjustments)
      row.adjustments = withMarkers(patch.adjustments ?? now.adjustments, {
        workflow: 'workflow' in patch ? patch.workflow : now.workflow,
        providerJob: 'providerJob' in patch ? patch.providerJob : now.providerJob,
      })
    }
  ...
  async reassignOwner(from: string, to: string): Promise<number> {
    ...
    const { data, error } = await supabase()
      .from(TABLE).update({ device_id: to }).eq('device_id', from).select('id')
    if (error) boom('reassign', error)
