...
import { parseByokModelId, type ByokProviderId } from '@/lib/byok/catalog'
import { BYOK_ERRORS } from '@/lib/byok/errors'
import { followVideoJob } from './videoJob'
...
export interface RunState {
  id: string
  /** What is being made, so views label stages correctly. */
  kind: 'video' | 'image'
  /**
   * True only for the in-browser encoder, which reports real per-frame
   * progress. Server jobs report stages, so their number is not a percentage.
   */
  measurable: boolean
...
class GenerationManager {
  ...
  private kinds = new Map<string, 'video' | 'image'>()
  private execs = new Map<string, 'client' | 'server'>()
  ...
  private setRun(id: string, stage: RunState['stage'], progress: number) {
    ...
    const kind = this.kinds.get(id) ?? 'video'
    const measurable = this.execs.get(id) !== 'server'
    this.commit({ runs: { ...this.snap.runs, [id]: { id, kind, measurable, stage, progress: next } } })
  }
  ...
  /**
   * Make a stored generation known to Studio views without re-fetching, e.g.
   * when Assets opens an older item that is beyond Studio's first page.
   */
  remember = (g: Generation): void => {
    if (this.snap.updated[g.id] === g) return
    this.publish(g)
  }
  ...
  enqueue = (gen: Generation, execution: 'client' | 'server' = 'client'): void => {
    this.kinds.set(gen.id, kindOfModel(gen.model))
    this.execs.set(gen.id, execution)
  ...
  cancel = async (id: string): Promise<void> => {
    ...
    // A provider render cannot be cancelled from Slate (no provider documents
    // it): say so, rather than pretend it stopped.
    const providerVideo = parseByokModelId(this.snap.updated[id]?.model ?? '')?.op === 'video'
    try {
      const { generation } = await api.patchGeneration(id, {
        event: 'fail', code: 'cancelled',
        message: providerVideo ? BYOK_ERRORS.byok_job_lost : 'Cancelled before the render finished.',
      })
  ...
  private async runServer(gen: Generation): Promise<void> {
    const own = parseByokModelId(gen.model)
    if (own?.op === 'video') return this.runProviderVideo(gen, own.providerId)
    ...
    try {
      // A job on the user's own key sends that key with this one request (and
      // nothing else ever does). Cancelling aborts it, which stops the
      // provider call on the server too. Without an active key the server
      // fails the job honestly instead of guessing.
      const own = parseByokModelId(gen.model)
      const byok = own ? byokStore.credentialBody(own.providerId) : null
      const { generation } = own
        ? await api.renderGeneration(gen.id, byok, ac.signal)
        : await api.renderGeneration(gen.id)
      this.publish(generation)
    } catch (err) {
  ...
  /**
   * A video on the user's own provider: submit once, then follow the
   * provider's asynchronous job from this tab, sending the key with each poll.
   * Following stops the moment the key is gone (sign-out, reload, a different
   * account), on cancel, and at a deadline; the job is then failed honestly,
   * saying the provider may still finish it. Nothing is resubmitted.
   */
  private async runProviderVideo(gen: Generation, providerId: ByokProviderId): Promise<void> {
    const ac = new AbortController()
    this.aborters.set(gen.id, ac)
    this.setRun(gen.id, 'image', 0)
    try {
      const { generation } = await api.renderGeneration(gen.id, byokStore.credentialBody(providerId), ac.signal)
      this.publish(generation)
      if (generation.status !== 'generating') return
      this.setRun(gen.id, 'render', generation.progress)

      const outcome = await followVideoJob({
        id: gen.id,
        credential: () => byokStore.credentialBody(providerId),
        poll: api.pollGeneration,
        signal: ac.signal,
        onUpdate: (g) => {
          this.publish(g)
          if (g.status === 'generating' && g.stage) this.setRun(gen.id, g.stage, g.progress)
        },
      })
      if (outcome === 'key_gone' || outcome === 'deadline' || outcome === 'lost') {
        try {
          const { generation: failed } = await api.patchGeneration(gen.id, {
            event: 'fail', code: 'byok_job_lost', message: BYOK_ERRORS.byok_job_lost,
          })
          this.publish(failed)
        } catch { /* already terminal, or signed out: the stale sweep settles it */ }
      }
    } catch (err) {
      if (ac.signal.aborted) return // cancel() has already recorded it
      try {
        const { generation } = await api.patchGeneration(gen.id, {
          event: 'fail', code: err instanceof ApiError ? err.code : 'render_failed',
          message: err instanceof Error ? err.message.slice(0, 500) : 'The render could not be started.',
        })
        this.publish(generation)
      } catch { /* server already marked it */ }
    } finally {
