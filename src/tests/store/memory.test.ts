...
import { MemoryBlobStore, MemoryGenerationStore } from '@/lib/store/memory'
import { STALE_AFTER_MS } from '@/lib/generation/state'
import type { NewGeneration } from '@/lib/store/types'
...
describe('MemoryGenerationStore', () => {
  ...
  it('reassigns ownership across jobs', async () => {
    await store.create(base('anon-1'))
    await store.create(base('anon-1'))
    await store.create(base('anon-2'))
    expect(await store.reassignOwner('anon-1', 'user:u1')).toBe(2)
    const list = await store.listByDevice('user:u1', { limit: 10 })
    expect(list.items).toHaveLength(2)
  })

  it('no-ops when from is empty or identical', async () => {
    await store.create(base('anon-1'))
    expect(await store.reassignOwner('', 'user:u1')).toBe(0)
    expect(await store.reassignOwner('anon-1', 'anon-1')).toBe(0)
  })
})
