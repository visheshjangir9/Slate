...
import { fileSize, timeAgo, timecode, downloadHref } from '@/lib/format'
...
describe('downloadHref', () => {
  it('asks Supabase Storage for an attachment', () => {
    const u = 'https://x.supabase.co/storage/v1/object/public/outputs/abc.mp4'
    expect(downloadHref(u, 'slate clip.mp4')).toBe(`${u}?download=slate%20clip.mp4`)
  })
  it('leaves same-origin URLs alone, where the download attribute works', () => {
    expect(downloadHref('/api/blob/abc.mp4', 'a.mp4')).toBe('/api/blob/abc.mp4')
  })
})
